...
export default function PaymentNavBar({ basePath = '' }: PaymentNavBarProps) {
  const navigate = useNavigate();
  const location = useLocation();
  
  const getActiveTab = () => {
    const path = location.pathname;
    if (path === basePath || path === `${basePath}/` || path === '/home' || path === '/') return 'home';
  ...
  return (
    <nav className="absolute bottom-0 left-0 right-0 bg-[#0d1025] border-t border-gray-800 px-4 py-2 z-50">
